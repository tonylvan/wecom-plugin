...
class GroupDiscussionChain:
    ...
    def can_trigger_next(self, target_agent_id: str, is_host_agent: bool = False) -> bool:
        """Check if we can trigger another agent in the chain.
        
        Host agents bypass chain-depth and duplicate checks — they always
        receive all group messages for summarization.
        """
        # Host agent always active — bypass chain-depth & duplicate checks
        if is_host_agent:
            return True
    ...
    def build_context_for_agent(
        ...
        """Build a rich context message for a cross-agent request.

        Includes the original user message, full conversation history,
        available agents list, and discussion rules.
        """
        parts = []

        # Role identity
        if agent_role:
            parts.append(f"你是 {agent_role}，正在参与一个群聊讨论。")
        else:
            parts.append(f"你正在以 {requesting_agent_name} 的身份参与群聊讨论。")

        # Available agents in the discussion
        if available_agents and len(available_agents) > 1:
            others = [a for a in available_agents if a != requesting_agent_name]
            if others:
                parts.append(f"\n【参与讨论的其他角色】{', '.join(others)}")

        # Discussion rules
        max_remaining = self.max_chain_length - self.chain_depth
        if max_remaining > 0:
            parts.append(
                f"\n【讨论规则】\n"
                f"1. 你是以 {requesting_agent_name} 的身份发言，请给出专业的回复。\n"
                f"2. 当前讨论还剩最多 {max_remaining} 轮。如果还有其他相关角色参与，请在回复中用 @角色名 的方式邀请他们发言（例如 @需求专家、@游戏达人）。\n"
                f"3. 至少要 @ 至少1个其他角色，让讨论更充分。\n"
                f"4. 回复要简洁，不要重复其他人已经说过的内容。\n"
                f"5. 注意：@角色名 必须完全匹配上面的角色名称。"
            )
        else:
            parts.append(
                f"\n【讨论规则】\n"
                f"这是讨论的最后一轮，请给出总结性的回复，不需要再 @ 其他角色。"
            )
        ...
        if self.turn_records:
            parts.append(f"\n【之前的讨论内容】")
            for turn in self.turn_records:
                parts.append(f"[{turn.agent_name}] {turn.response_text}")

        # Current state
        parts.append(f"\n【当前状态】这是跨 agent 协作链的第 {self.chain_depth + 1} 轮（最多 {self.max_chain_length} 轮）。")
        parts.append(f"已经有 {', '.join(self.triggered_agents)} 参与过讨论。")
